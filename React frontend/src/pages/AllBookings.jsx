...
const BookingsPage = () => {
    ...
    const fetchRentalRecords = async () => {
        try {
            ...
            console.log("Rental records:", response.data);
        } catch (error) {
    ...
    const calculateTotalCost = (startDate, endDate, perDayRental, helmetCharge = 0) => {
        const start = new Date(startDate);
        const end = new Date(endDate);
        const days = (end - start) / (1000 * 60 * 60 * 24);
        return (days * perDayRental) + helmetCharge;
    };
            ...
            <table style={styles.table}>
                <thead>
                    <tr>
                        ...
                        <th style={styles.th}>Total Cost (incl. Helmet)</th>
                        ...
                        <tr key={record.rentalRecordId} style={styles.tr}>
                            ...
                            <td style={styles.td}>
                                Rs.{calculateTotalCost(
                                    ...
                                    record.bike.perDayRental,
                                    record.extraHelmet || 0
                                ).toFixed(2)}
                            </td>
                            <td style={styles.td}>
                                <button 
